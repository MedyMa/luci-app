...
const SlideAnimations = {
	...
	slideDown: function (element, duration, callback) {
		...
		const ms = this._resolveDuration(duration);
		...
		// Cleanup on transitionend (primary), setTimeout as fallback
		const cleanup = this._makeCleanup(element, true, callback);
	...
	slideUp: function (element, duration, callback) {
		...
		const cleanup = this._makeCleanup(element, false, callback);
		element._slideCleanup = cleanup;
		element.addEventListener('transitionend', cleanup, { once: true });
		element._slideFallback = setTimeout(cleanup, ms + 80);
	},
	...
	/**
	 * Stop running animation on element
	 */
	_stop: function (element) {
		...
		// Cancel any in-flight CSS transition
		element.style.transition = '';
		element.offsetHeight; // force reflow to apply
	},

	/**
	 * Create cleanup closure with DOM detachment guard
	 */
	_makeCleanup: function (element, isOpen, callback) {
		return function cleanup() {
			...
			if (typeof callback === 'function') {
				...
				catch (e) { console.error('SlideAnimations callback error:', e); }
			}
		};
	},
